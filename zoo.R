...
library(mlbench)
library(lattice)
zoo <- read.csv('C:\\Users\\WIN10\\Desktop\\LEARNING\\DS\\Zoo.csv')
str(zoo)
zoo1 <- zoo[,2:18]
str(zoo1)
...
# Splitting the data
set.seed(123)
spl <- sample(2,nrow(zoo1), replace = T, prob = c(0.7,0.3))
train <- zoo1[spl==1,]
test <- zoo1[spl==2,]
# KNN Model 
trcontrol <- trainControl(method = "repeatedcv", number = 10,repeats = 3) # classprobs are needed when u want to select ROC for optimal K Value
set.seed(222)
